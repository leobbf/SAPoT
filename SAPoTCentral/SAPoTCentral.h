...
int SAPoTCentral_error();
					...
					/************************* Functions for MQTT **************************/
/**
* Essa função foi implementada através das ferramentas disponibilizadas pela biblioteca MQTTClient.h que fora desenvolvida pelo 
* projeto Eclipse Paho. Essa função tem o intuito de conectar a Central SAPoT ao broker MQTT, através de um objeto MQTTClient,
* da seguinte maneira: Primeiramente, ela verifica a existência de conexão com o broker e se caso a conexão não exista, ela destrói 
...
* Em sequência o MQTTClient requisita a conexão com o broker via 
...
/**
* Essa função foi implementada com base na documentação da biblioteca MQTTClient.h do projeto Eclipse Paho. Ela possui um padrão de 
* argumentos definidos para manipular as mensagem recebidas via MQTT, quando a função MQTTClient_setCallbacks() configura a recepção 
...
/**
* Publica a mensagem em um topico especifico no broker MQTT
*
*/
